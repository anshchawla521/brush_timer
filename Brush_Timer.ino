...
void loop() {
  if (is_running) {
    if (millis() - start_time > ((unsigned long)tim * 1000)) { // overflow automatically taken care of
      // time up
      is_running = false;
    } else {
      // running and time not over
      time_left = int(float(minpulsetime) + float(maxpulsetime - minpulsetime) *  (float((start_time + ((unsigned long)tim * 1000) - millis()) / 1000.0) / float(tim))   ) ; // find max limit of the operations
